...
from model.ResNet50_ProvKD_wOriginal_RealTime import ResNetProvKD_RealTime
from model.ResNet50_UNI_wOriginal_RealTime import ResNetUNI_RealTime
...
# Initialize datasets
train_dataset = RealTimeCroppedDualPatchDataset(image_dir=TRAIN_DIR, data_path=train_data_path, mode='training')  # Add transforms if needed
val_dataset = RealTimeCroppedDualPatchDataset(image_dir=VAL_DIR, data_path=valid_data_path, mode='valid')      # Add transforms if needed
...
model = ResNetProvKD_RealTime(num_classes=2)
# checkpoint_path = '/home/manhduong/ISBI25_Challenge/Giloma-MDC25/src/checkpoints/best_model_Resnet_ProvKD_Dual_pretrain_augment_realtime_crop_local_context_wO.pth'
...
def freeze_modules(model):
    # blacklist = ['classifier', 'reweight', 'main_enc']
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels, context in tqdm(dataloader, total=len(dataloader)):
                ...
                if phase == 'train':
                    ...
                    all_wrong_instances += torch.sum(wrong_instances).detach().cpu().item()
                    ...
                    wrong_probs = torch.sum(probs * mask)
                    ...
                    with torch.no_grad():
                        ...
                        wrong_instances = 1 - (preds == labels.data).int()
                        wrong_probs = torch.sum(probs * wrong_instances) 
            ...
            all_wrong_probs /= (all_wrong_instances + 1e-9)
            ...
            if phase=='val':
                if epoch_f1 > best_f1:
                    best_f1 = epoch_f1
                    best_wrong_probs = all_wrong_probs  # reupdate
                    torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, f'best_model_{name}_wO.pth'))
                    print(f"Best model updated at epoch {epoch + 1} | Reupdate WrongPrb")
                elif abs(epoch_f1 - best_f1) < 1e-8:
...
if __name__ == '__main__':
    ...
    name = 'Resnet_Prov_Dual_pretrain_augment_realtime_crop_local_context'
    # Start training
